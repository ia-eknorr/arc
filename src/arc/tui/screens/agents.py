...
class AgentDetail(Static):
    ...
    def show(self, name: str) -> None:
        ...
        lines.append(f"  model:             {data.get('model', '')}  [dim](c: change)[/dim]")
        ...
        lines.append(f"  permission_mode:   {data.get('permission_mode', '')}")

        spf = data.get("system_prompt_files", [])
        if spf:
            lines.append("  system_prompt_files:")
            for f in spf:
                lines.append(f"    {f}")

        ch = data.get("discord", {}).get("channel_id", "")
        if ch:
            lines.append(f"  discord channel:   {ch}")

        lines.append("")
        lines.append("[dim]e: open in editor  c: change model  d: delete[/dim]")
        self.update("\n".join(lines))
...
class AgentsPane(Widget):
    ...
    def compose(self) -> ComposeResult:
        with Horizontal():
            with Vertical(id="agents-list-pane"):
                yield Label("[bold]AGENTS[/bold]", classes="section-title")
    ...
    def _selected_name(self) -> str | None:
        ...
        if idx is None or not self._agents:
            return None
        return self._agents[idx].name if idx < len(self._agents) else None
    ...
    def action_new_agent(self) -> None:
        async def _do() -> None:
            ...
            if not workspace:
                return
            model = await self.app.push_screen_wait(InputScreen("Model:", "claude-sonnet-4-6"))
            if not model:
                return
